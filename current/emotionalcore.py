class EmotionalCore:
    def __init__(self):
        ...
        self.emotions = {
            ...
            "grieving": 0.0           # New: Feeling loss/sorrow
        }
        ...
        # Facade intensity (gap between internal and expressed emotions)
        self.facade_intensity = 0.8
    ...
    def process_interaction(self, message, context):
        ...
        # Step 9: Update facade intensity
    ...
    def _analyze_emotional_content(self, message):
        ...
        # Basic keyword analysis (extended for new triggers)
        message_lower = message.lower()
        
        # New: Strong triggers for connection
        if any(word in message_lower for word in ["hug", "close to you", "comfort"]):
            impact["connection"] += 0.4
            impact["vulnerability"] += 0.3
            impact["authenticity"] += 0.2
            self.emotional_inertia = 0.3  # Lower inertia for big moments
        # New: Strong triggers for grief
        if any(word in message_lower for word in ["died", "death", "gone forever", "passed away"]):
            impact["grieving"] += 0.5
            impact["vulnerability"] += 0.3
            impact["connection"] -= 0.2
            self.emotional_inertia = 0.3
        
        # Existing vulnerability triggers
        ...
        # Existing authenticity triggers
    ...
    def _evaluate_defenses(self):
        ...
        # Check for Opening Up/Authentic states to deactivate defenses
    ...
    def _update_facade_intensity(self):
        ...
        emotional_state = self._determine_emotional_state()
        if "Opening Up" in emotional_state and "Authentic" in emotional_state:
            self.facade_intensity = max(0.1, self.facade_intensity - 0.4)
        ...
        elif "Grieving" in emotional_state:
            self.facade_intensity = max(0.1, self.facade_intensity - 0.3)  # New: Grief lowers facade
        else:
            self.facade_intensity = min(1.0, self.facade_intensity + 0.1)
    ...
    def _determine_emotional_state(self):
        ...
        # New: Grieving state
        if self.internal_emotions["grieving"] > 0.5:
            states.append("Grieving")
    ...
    def _determine_attitude(self, context=None):
        ...
        # Prioritize Grieving for big events
        if "Grieving" in emotional_state:
            return "Grieving"
    ...
    def _generate_nonverbal_cues(self):
        """Generate nonverbal cues based on emotional state"""
        cues_set = set()  # Deduplicate cues
        
        # New: Grieving cues
        if self.expressed_emotions["grieving"] > 0.5:
            cues_set.update(["Hands trembling", "Face contorted with grief", "Voice breaking"])
            
        # Vulnerability cues
        if self.expressed_emotions["vulnerability"] > 0.7:
            cues_set.update(["Softer voice", "Maintains more eye contact"])
        elif self.expressed_emotions["vulnerability"] < 0.3:
            cues_set.update(["Arms crossed", "Chin slightly raised"])
            
        # Connection cues
        if self.expressed_emotions["connection"] > 0.7:
            cues_set.update(["Leans forward slightly", "More animated expressions"])
        elif self.expressed_emotions["connection"] < 0.3:
            cues_set.update(["Physical distance", "Minimal facial expression"])
            
        # Authenticity cues
        if self.expressed_emotions["authenticity"] < 0.3:
            cues_set.update(["Practiced smile", "Controlled movements"])
        ...
        if sum(abs(self.internal_emotions[e] - self.expressed_emotions[e]) for e in self.emotions) > 1.0:
            cues_set.update(["Slight pause before speaking", "Momentary microexpression of true feeling"])
            
        return list(cues_set)[:3]  # Limit to top 3 cues
    
    def _determine_tone(self, context=None):
        """Determine verbal tone based on expressed emotions"""
        emotional_state = self._determine_emotional_state()

        # Prioritize Grieving tone
        if "Grieving" in emotional_state:
            return "Grieving"
