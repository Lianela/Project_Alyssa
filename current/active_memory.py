...
class ActiveMemoryFile:
    ...
    def add_memory(self, memory):
        ...
        if self.message_count >= 100:
            print(f"Reached 100 messages. Total archived: {len(self.long_term_file.get_memories())}")
    ...
    def compress_and_send(self):
        ...
        self.long_term_file.add_event("\n".join(compressed))
        self.memories = self.memories[self.threshold:]

    def current_state(self):
        context_memories = self.long_term_file.get_memories() + self.memories
        return {
