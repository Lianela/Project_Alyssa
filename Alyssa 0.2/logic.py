# logic.py (Option 2 Impl + Detailed Logging + Dynamic Time + Persistence)
import logging
import re
import time # Needed for real time tracking
import datetime # Needed for roleplay time tracking
import json # Needed for saving/loading state
import os   # Needed for checking if save file exists

# Import component classes only if type hinting is needed, instances are passed in __init__
...
class RPLogic:
    def __init__(self, character_memory, active_memory, user_memory, emotional_core, dynamic_memory):
        ...
        # Ensure ActiveMemoryFile correctly initializes its LTM instance
        if hasattr(self.active_memory, 'long_term_file') and self.active_memory.long_term_file:
            ...
            # Fallback or error handling if LTM isn't accessible via active_memory
            logic_logger.error("LongTermMemoryFile instance not found via ActiveMemoryFile. LTM will not function correctly.")
            # You might want to initialize a default LTM here or raise an error
        ...
        self.logger = logic_logger # Use logic_logger for general logic messages
        # Regex to find action between the first pair of asterisks
        self.action_regex = re.compile(r"\*(.*?)\*", re.DOTALL)

        # --- Default Initial State ---
        # These will be potentially overwritten by _load_state()
        self.current_roleplay_time = datetime.datetime(2025, 4, 16, 14, 0, 0) # Default start time
        self.last_real_time = time.time() # Store the real time when the logic starts
        # Configuration for time advancement (consider moving to config file later)
        self.time_scale_factor = 15 # 1 real second = 15 roleplay seconds (Adjustable)
        self.min_time_advance_per_turn = datetime.timedelta(minutes=2) # Advance at least 2 mins per turn (Adjustable)
        # Default states for memories and action are handled by their respective classes' __init__
    ...
    def _load_state(self):
        ...
        if os.path.exists(SAVE_STATE_FILE):
            ...
            try:
                ...
                self.logger.debug("Loaded raw state data: %s", state_data) # Log loaded data for debug

                # Restore Time
                time_str = state_data.get("current_roleplay_time")
                if time_str:
                    try:
                        # Attempt parsing with timezone awareness if saved with it, otherwise naive
                        if '+' in time_str or 'Z' in time_str:
                             self.current_roleplay_time = datetime.datetime.fromisoformat(time_str)
                        else:
                             # Assume naive datetime if no timezone info
                             loaded_time = datetime.datetime.fromisoformat(time_str)
                             # Decide how to handle timezone - make it timezone-aware using local timezone?
                             # Or keep it naive if consistency is maintained? Let's keep it naive for now.
                ...
                if emo_state:
                    ...
                    # Use getattr/setattr for robustness or direct access if structure is stable
                    self.emotional_core.fatigue_level = emo_state.get("fatigue_level", self.emotional_core.fatigue_level)
                    self.emotional_core.current_trust = emo_state.get("current_trust", self.emotional_core.current_trust)
                    self.emotional_core.current_intimacy = emo_state.get("current_intimacy", self.emotional_core.current_intimacy)
                    # Ensure time_since_last_sleep is loaded correctly (it's a float/int)
                    ...
                    # Example for relationship distance if added later:
                    # self.emotional_core.relationship_distance = emo_state.get("relationship_distance", self.emotional_core.relationship_distance)

                    self.logger.info("Loaded key emotional state attributes (Fatigue: %.2f, Trust: %.2f, Intimacy: %.2f, Time Since Sleep: %.1f).",
                                    self.emotional_core.fatigue_level, self.emotional_core.current_trust,
                                    self.emotional_core.current_intimacy, self.emotional_core.time_since_last_sleep)

                # Reset last_real_time to now, as the real time delta calculation
                # should start from when the script is loaded now.
                ...
                # Catch more potential errors during loading
                self.logger.error("Failed to load state from '%s': %s. Starting with default state.", SAVE_STATE_FILE, e, exc_info=True)
                # Ensure defaults are set if loading fails partially (optional, __init__ already set them)
        else:
    ...
    def _save_state(self):
        ...
        try:
            ...
            state_data = {
                "current_roleplay_time": self.current_roleplay_time.isoformat(), # Save as ISO string
                ...
                "emotional_core_state": {
                    # Save key simple attributes
                    "fatigue_level": self.emotional_core.fatigue_level,
                    "current_trust": self.emotional_core.current_trust,
                    "current_intimacy": self.emotional_core.current_intimacy,
                    "time_since_last_sleep": self.emotional_core.time_since_last_sleep,
                    # Add other simple state variables here if desired
                    # e.g., "relationship_distance": self.emotional_core.relationship_distance
                }
                # Add other state components if needed (e.g., character personality if it evolves)
            }
            self.logger.debug("State data prepared for saving: %s", state_data) # Log data before saving
            ...
            with open(temp_save_file, 'w', encoding='utf-8') as f:
                json.dump(state_data, f, indent=4, ensure_ascii=False) # Use indent and ensure_ascii=False

            # Rename temporary file to the actual save file (atomic on most systems)
            os.replace(temp_save_file, SAVE_STATE_FILE) # Replaces existing file
    ...
    def construct_context(self, user_input):
        ...
        # 2. Get previous narrative action
        ...
        # 3. Get current roleplay time
        current_time_str = self.current_roleplay_time.strftime("%A, %I:%M %p") # Example: "Wednesday, 02:30 PM"
        self.logger.debug("Current roleplay time for context: %s", current_time_str)

        # 4. Determine Context Flags
        self.logger.debug("Determining context flags...")
        context_flags = {
    ...
    def manage_dynamic_memory(self, user_input, full_ai_response):
        ...
        self.last_real_time = current_real_time # Update for next turn
        ...
        if match:
            ...
            self.logger.debug("Action extracted using regex: %s", extracted_action)
             ...
             # Improved heuristic for fallback action extraction
